...
class InteriorLivingRoomWallColorScreen extends StatefulWidget {
  static const String routeName = '/interiorLivingroomWallColor';

  @override
  _InteriorLivingRoomWallColorScreenState createState() =>
      _InteriorLivingRoomWallColorScreenState();
}

class _InteriorLivingRoomWallColorScreenState
    extends State<InteriorLivingRoomWallColorScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    if (designDataProvider.oceanBuilder.livingRoominteriorWallColor != null) {
      if (designDataProvider.oceanBuilder.livingRoominteriorWallColor
              .compareTo(ListHelper.getWallColorList()[0]) ==
          0)
        _bloc.sink
            .add(designDataProvider.oceanBuilder.livingRoominteriorWallColor);
      else {
        _bloc.sink.add(list[1]);
        _selectedColor.sink.add(int.parse(
            designDataProvider.oceanBuilder.livingRoominteriorWallColor));
  ...
  Appbar _topBar() {
    return Appbar(
            ScreenTitle.INTERIOR_LIVING_ROOM_WALL_COLOR,
            isDesignScreen: true,
          );
  }
  ...
  goNext(DesignDataProvider designDataProvider) {
    ...
    Navigator.of(context)
        .pushNamed(InteriorMasterBedRoomWallColorScreen.routeName);

    // debugPrint(
    // designDataProvider.oceanBuilder.livingRoominteriorWallColor.toString());
  }
