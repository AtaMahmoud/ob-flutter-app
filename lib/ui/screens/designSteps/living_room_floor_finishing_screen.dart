...
class LivingRoomFloorFinishingScreen extends StatefulWidget {
  static const String routeName = '/livingRoomFloorFinishing';

  @override
  _LivingRoomFloorFinishingScreenState createState() =>
      _LivingRoomFloorFinishingScreenState();
}

class _LivingRoomFloorFinishingScreenState
    extends State<LivingRoomFloorFinishingScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    if (designDataProvider.oceanBuilder.livingRoomloorFinishing != null) {
      if (designDataProvider.oceanBuilder.livingRoomloorFinishing
              .compareTo(ListHelper.getFloorFinishingColorList()[0]) ==
          0) {
        _bloc.sink.add(designDataProvider.oceanBuilder.livingRoomloorFinishing);
        _floorFinishingMaterial =
            designDataProvider.oceanBuilder.livingRoomloorFinishing;
      } else {
        List<String> splits =
            designDataProvider.oceanBuilder.livingRoomloorFinishing.split(',');
        if (splits.length == 2) {
  ...
  Appbar _topBar() {
    return Appbar(
            ScreenTitle.LIVING_ROOM_FLOOR_FINISHING,
            isDesignScreen: true,
          );
  }
  ...
  goNext(DesignDataProvider designDataProvider) {
    ...
    Navigator.of(context).pushNamed(KitchenFloorFinishingScreen.routeName);

    // debugPrint(designDataProvider.oceanBuilder.livingRoomloorFinishing.toString());
  }
