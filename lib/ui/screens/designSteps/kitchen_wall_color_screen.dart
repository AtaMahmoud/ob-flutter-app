...
class InteriorKitchenWallColorScreen extends StatefulWidget {
  static const String routeName = '/interiorKitchenWallColor';

  @override
  _InteriorKitchenWallColorScreenState createState() =>
      _InteriorKitchenWallColorScreenState();
}

class _InteriorKitchenWallColorScreenState
    extends State<InteriorKitchenWallColorScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    if (designDataProvider.oceanBuilder.kitcheninteriorWallColor != null) {
      if (designDataProvider.oceanBuilder.kitcheninteriorWallColor
              .compareTo(ListHelper.getWallColorList()[0]) ==
          0)
        _bloc.sink
            .add(designDataProvider.oceanBuilder.kitcheninteriorWallColor);
      else {
        _bloc.sink.add(list[1]);
        _selectedColor.sink.add(int.parse(
            designDataProvider.oceanBuilder.kitcheninteriorWallColor));
      ...
      child: Scaffold(
        body: Stack(
          children: <Widget>[
            ...
            Appbar(
              ScreenTitle.INTERIOR_KITCHEN_WALL_COLOR,
              isDesignScreen: true,
            ),
            Positioned(
                bottom: 0,
  ...
  goNext(DesignDataProvider designDataProvider) {
    ...
    Navigator.of(context).pushNamed(InteriorBedRoomWallColorScreen.routeName);
    // debugPrint(designDataProvider.oceanBuilder.kitcheninteriorWallColor.toString());
  }
