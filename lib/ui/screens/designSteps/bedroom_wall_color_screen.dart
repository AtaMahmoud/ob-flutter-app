...
class _InteriorBedRoomWallColorScreenState
  ...
  Widget build(BuildContext context) {
    ...
    if(designDataProvider.oceanBuilder.interiorBedroomWallColor != null){
      if(designDataProvider.oceanBuilder.interiorBedroomWallColor.compareTo(ListHelper.getWallColorList()[0])==0)
        _bloc.sink.add(designDataProvider.oceanBuilder.interiorBedroomWallColor);
       else{
         _bloc.sink.add(list[1]);
         _selectedColor.sink.add(int.parse(designDataProvider.oceanBuilder.interiorBedroomWallColor));
      ...
      child: Scaffold(
        body: Stack(
          children: <Widget>[
            ...
            Appbar(ScreenTitle.INTERIOR_BEDROOM_WALL_COLOR,isDesignScreen: true,),
            Positioned(
                bottom: 0,
                left: 0,
                right: 0,
  ...
  goNext(DesignDataProvider designDataProvider) {
    ...
    if(_wallColor.compareTo(ListHelper.getWallColorList()[0])==0){
      designDataProvider.oceanBuilder.interiorBedroomWallColor = '0xffffffff';//_wallColor;
    }else{
      designDataProvider.oceanBuilder.interiorBedroomWallColor = _wallColorCustom;
    }
